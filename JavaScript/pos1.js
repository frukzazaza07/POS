...
let checkLoginStatus = sessionStorage.getItem("sessionLogin");
...
$(document).ready(function () {
    ...
    var pageNumberList = new Array();
    var test1 = new Array();
    var testtemplate;
    var sessionT = sessionStorage.getItem("sessionT");
    var obj = {
        productId: "1",
        productName: "test",
        productPrice: 555,
    ...
    //loadtemplate
    let checkType = sessionStorage.getItem("sessionT");
    let templateNav = localStorage.getItem("sendTemplate");
    var headNav = document.querySelector(".header-container-grid");
    if (checkType == "admin") {

        headNav.style.gridTemplateColumns = "repeat(6, 10%) 1fr";
        $(".header-container-grid").append(templateNav);
    } else {
        headNav.style.gridTemplateColumns = "repeat(4, 10%) 1fr";
        $(".header-container-grid").append(templateNav);
    }
    ...
    $.ajax({
        ...
        success: function (dataResult) {
            ...
            for (x = 0; x < data["data"].length; x++) {
                //for (y = 0; y < data["data"][x].length; y++) {}
                //  console.log(data["data"][x][y]);
                var obj = {
                    productId: "" + data["data"][x][0],
                    productName: "" + data["data"][x][2],
                    productPrice: data["data"][x][3],
                    productQuantity: data["data"][x][4]
                };
                product.push(obj);
            ...
            for (i = 0; i < data["pageNo"].length; i++) {

                pageNumberList.push(data["pageNo"][i]);
                ...
                /*
                var li_pageNumber = document.createElement("LI");
                li_pageNumber.innerHTML = data["pageNo"][i];
                $(".pageNumber").append(li_pageNumber);
    ...
    setTimeout(function () {
        let elePosition = document.querySelector("#head-position");
        let eleFullname = document.querySelector("#head-fullname");
        let sessionFn = sessionStorage.getItem("sessionFn");
        let sessionLn = sessionStorage.getItem("sessionLn");
        let sessionP = sessionStorage.getItem("sessionP");
        let sessionU = sessionStorage.getItem("sessionU");
        eleFullname.innerHTML = sessionFn + " " + sessionLn;
        elePosition.innerHTML = sessionP;
        ...
        function changeProductBtn(product) {
            /*var product = new Array();
            product = productg;*/
            ...
            for (var i = 0; i < product.length; i++) {
                ...
                if (product[i] == null) {
                    ...
                    var template = "<div class='div-btn-menu'><button id='btn-menu' class='btnmenu btn-secondary' value='" + product[i].productId + "'>" + product[i].productName + "</button></div> ";
            ...
            $('.btnmenu').click(function () {
                ...
                //alert(product[valueOfButton].productId);
                //alert(valueOfButton);
                console.log(product);
                valueOfButtonInt = parseInt(valueOfButton);
                //  alert(valueOfButtonInt);
        ...
        for (i = 0; i < pageNumberList.length; i++) {
            var liTemplate = "<li class='numberList' ><p class='linknumberList' id='linknumberList' >" + pageNumberList[i] + "</p></li>";
            $(".pageNumber").append(liTemplate);
        }
        ...
        /*var product = '[{"productId": "0","productName": "ปูไข่ดอง","productPrice": 200, "productQuantity": 1}]';
        var obj = JSON.parse(product);
        obj.push({ "productId": "1", "productName": "ปูไข่ดอง", "productPrice": 200, "productQuantity": 1 });
        ...
        console.log(test);
        //var product = '[{"productId":"1","productName":"ปูไข่ดอง","productPrice":200,"productQuantity":1}]';


        //alert(product);

        //obj.push({ "productId": "2", "productName": "กุ้งเผา", "productPrice": 500, "productQuantity": 1 });
        // product = JSON.stringify(obj);
        ...
        */
        ...
        /******************************************** ตัวแปร array product ข้างบนมาเป็นก้อนไม่เป็นขนาด*/
        /*var template1 = "<div class='div-btn-menu'> \
        <button id='btn-menu' class='btnmenu btn-secondary' value='" + product1.productId + "'>" + product1.productName + "</button></div> ";
        $("#grid-menu-button").append(template1);*/
        ...
        for (var i = 0; i < product1.length; i++) {
            var template = "<div class='div-btn-menu'> \
        <button id='btn-menu' class='btnmenu btn-secondary' value='" + product1[i].productId + "'>" + product1[i].productName + "</button></div> ";
            $("#grid-menu-button").append(template);
        }
        ...
        */
        ...
        function discount() {
            ...
            $('#btn-discount-submit').on("click", function (e) {
                ...
                errorDiscount1 = document.getElementById("error-discount1")
                errorDiscount2 = document.getElementById("error-discount2")
                if ((txtDiscount1 == "" && discount1 == true) || (txtDiscount2 == "" && discount2 == true)) {
                    if (discount1 == true) {
                        errorDiscount1.innerHTML = "*ระบุตัวเลข";
        ...
        $("#btn-confirm").on("click", function (e) {
            if (finish == true && discountValue == true) {
                ...
                console.log(menuQuantityPhp);
                console.log(menuCheckPhp);
                $.ajax({
                    url: "php/insert.php",
                    type: "POST",
                    data: {
        ...
        $(".linknumberList").click(function () {

            $(".linknumberList").attr('style', 'color:black');
            //document.getElementById("linknumberList").style.color = "black";
            var afterClick = $(this);
            afterClick.attr('style', 'color:red');
            var pageNoClick = $(this).text();
            $.ajax({
                url: "php/changeproduct.php",
                type: "GET",
                data: {
                    pageNumber: pageNoClick,
                },
                ...
                success: function (dataResult) {
                    ...
                    for (x = 0; x < data["data"].length; x++) {
                        ...
                        //product.push(obj1); *************ใช้------------
                        // product.splice(data["data"][x][0], 0, obj1);
                        product[data["data"][x][0]] = obj1;
